...
{
    public partial class captura_documento : System.Web.UI.Page
    {
        ...
        protected void CrearNuevoDocumentoYContinuar()
        {
            ...
            bool Puede = true;
            bool foliorepetido = true;
            /* VALIDACIONES  025AB */
            //PRINCIPAL, QUE NO SEA UNA REPETICIÓN

            //var checarfolio = ctx.tbExpedActDocumentos.Where(a => a.numeroDocumento == txtNumDocto.Text.ToUpper()).FirstOrDefault();
            //if (checarfolio != null)
            //{
            //    foliorepetido = false;
            //}
            ...
            ////if (!string.IsNullOrEmpty(hfPrevioNumDocto.Value) && !string.IsNullOrEmpty(hfPrevioAsunto.Value) && !string.IsNullOrEmpty(txtNumDocto.Text) && !string.IsNullOrEmpty(txtAsuntoDoc.Text))
            //if (!foliorepetido)
            //{
            //    string tMsj = "";
            //    tMsj = " NÚMERO:  " + txtNumDocto.Text.ToUpper();
            //    //if (hfPrevioAsunto.Value.ToUpper() == txtAsuntoDoc.Text.ToUpper())
            //    //    tMsj += "TEXTO DEL ASUNTO";
            //    //NOTIFICAR QUE SE DEBE LLENAR MÁS INFO

            //    Puede = false;
            //    //tMsj = Regex.Replace(tMsj, @"\sY\s$", "");
            //    lblMeNSAJE.Text = $"<h6 style='color:red'>YA HA CREADO PREVIAMENTE EL DOCUMENTO CON EL MISMO {tMsj}</h4><br /><b> .SE HABÍAN CONSERVADO LOS DATOS PARA FACILITAR LA SIGUIENTE CAPTURA. DEBE MODIFICAR AL MENOS EL NÚMERO DEL DOCUMENTO Y EL ASUNTO PARA CREAR OTRO. SI REQUIERE FORMULARIO LIMPIO, SELECCIONE LA OPCIÓN 'NUEVO DOCUMENTO' DEL MENÚ</b>";
            //    //mpe_msj.Show();
            ...
            {
                //GUARDAR DATOS DE LA PROCEDENCIA
                ...
                try
                {
                    ...
                    nuevaActuacion.idOrganismoDestino = idAreaAsignada;
                    nuevaActuacion.idAreaJuridica = idAreaAsignada;
                    ...
                    //GUARDAR LOS TIPOS Y SUBTIPOS DE ASUNTO, CON SUS DETALLES ADICIONALES
                    ...
                    foreach (tbExpedActPersonas relPer in listaPersonas)
                    {
                        ...
                        nuevaActuacion.tbExpedActPersonas.Add(new tbExpedActPersonas()
                        {
                            idPersona = idPersona,
                            idFigura = relPer.idFigura,
                            C_fechaReg = DateTime.Now
                        });
                    }
                    nuevaActuacion.C_fechaReg = DateTime.Now;
                ...
                if (nuevaActuacion.id > 0)
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(hfidPersRecib.Value) && !string.IsNullOrEmpty(textoNombrePersonaRecibe))
                        {
                            ...
                            nuevaRelEAP.idPersona = Convert.ToInt32(hfidPersRecib.Value);
                        ...
                        nuevaActuacion.tbExpedActDocumentos.Add(nuevoDocumento);
                        nuevaActuacion.accion = nuevoDocumento.asunto;
                        ...
                        adminSeg.guardarSegimientoCreacion(AdminSeguimiento.AccionRealizada.CREACION_DOCUMENTO, "tbExpedActuaciones", nuevaActuacion.id.ToString(), "", nuevaActuacion.C_fechaReg.Value, User.Identity.Name);

                        hfPrevioNumDocto.Value = datoPrevioND;
                        hfPrevioAsunto.Value = datoPrevioAsunto;

                        Guid IdExpedienteGuid = Guid.NewGuid();
                        tbExpedientes tbExped = new tbExpedientes();
                        tbExped.id = IdExpedienteGuid;
                        tbExped.numero = folio;
                        ...
                        //ScriptManager.RegisterStartupScript(this, GetType(), "Volver", "cambioTabBoton('divSeccion_Datos');", true);

                        var mensaje = $"<div style='width:100%; text-align:center; font-weight:bold;'>¡DOCUMENTO REGISTRADO CON ÉXITO!</div><hr />FOLIO INTERNOS: {folio} <b>, AHORA PUEDE CONSULTAR EL DOCUMENTO POR SU NÚMERO INGRESADO: <b>{nuevaActuacion.tbExpedActDocumentos.FirstOrDefault().numeroDocumento}</b> EN EL BUSCADOR DE DOCUMENTOS. {txt}";
                    ...
                    catch (Exception ex)
                    {
                        ...
                        //OCURRIÓ UN PROBLEMA AL REGISTRAR DATOS DE LOS DOCUMENTOS, POR LO QUE SE DEBE ELIMINAR EL REGISTRO DE LA ACTUACIÓN
                        var mensjae = $"<h3>PROBLEMA AL COMENZAR EL REGISTRO</h3><p>OCURRIÓ EL SIGUIENTE PROBLEMA [X002], POR LO QUE NO SE HA GUARDADO EL REGISTRO: {ex.Message} | {(ex.InnerException != null ? ex.InnerException.Message : "")} | {(ex.InnerException != null && ex.InnerException.InnerException != null ? ex.InnerException.InnerException.Message : "")}</p>";
                        //mpe_msj.Show();
                        ScriptManager.RegisterStartupScript(this, GetType(), "MostarModalMensajes", "MostarModalMensajes();", true);
                        lblTitulo.InnerText = "OCURRIÓ UN PROBLEMA";
                        lblMeNSAJE.Text = mensjae;
            ...
            else
            {
                ...
                //NOTIFICAR QUE SE DEBE LLENAR MÁS INFO
                //lbl_msj.Text = $"<h3>FAVOR DE LLENAR LO SIGUIENTE</h3><ul>{textoValidaciones}</ul>";
                //mpe_msj.Show();
            }
            //}
        }
